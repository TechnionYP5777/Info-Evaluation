...
public class SqlQueriesRunner {
	...
	/*
	public List<TableEntry> getBornInPlaceBeforeYear(String year,String place) throws Exception {
		//Connector conn = new Connector();
		int yearVal = Integer.parseInt(year);
		final String yearPlaceQuery = "SELECT basic_info.name,BirthDate,wikiPageID FROM basic_info,WikiID "
				+ "WHERE YEAR(BirthDate) < 1993 AND BirthPlace LIKE '%a%' "
				+ "AND wikiPageID = (SELECT wikiPageID FROM WikiID WHERE WikiID.name = basic_info.name LIMIT 1) LIMIT 5";
	...
	public List<TableEntry> getDifferentDeathPlace() throws Exception {
		//Connector conn = new Connector();
